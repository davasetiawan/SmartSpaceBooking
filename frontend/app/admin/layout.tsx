    ...
    <div className="admin-app-wrapper">
      ...
      <aside className={`admin-sidebar ${sidebarCollapsed ? 'collapsed' : ''}`}>
        ...
        <div className="sidebar-menu">
          ...
          <Link href="/admin" className={`menu-item ${isActive('/admin') && pathname === '/admin' ? 'active' : ''}`}>
            <LayoutDashboard size={18} />
            {!sidebarCollapsed && <span>Dashboard Overview</span>}
          </Link>

          <Link href="/admin/reservasi" className={`menu-item ${isActive('/admin/reservasi') ? 'active' : ''}`}>
            <CalendarCheck size={18} />
            {!sidebarCollapsed && <span>Kelola Reservasi</span>}
          </Link>

          <Link href="/admin/ruangan" className={`menu-item ${isActive('/admin/ruangan') ? 'active' : ''}`}>
            <Building2 size={18} />
            {!sidebarCollapsed && <span>Kelola Ruangan</span>}
          </Link>

          <Link href="/admin/users" className={`menu-item ${isActive('/admin/users') ? 'active' : ''}`}>
            <Users size={18} />
            {!sidebarCollapsed && <span>Kelola Member</span>}
          </Link>

          <Link href="/admin/scanner" className={`menu-item ${isActive('/admin/scanner') ? 'active' : ''}`}>
            <QrCode size={18} />
          ...
          <Link href="/admin/laporan" className={`menu-item ${isActive('/admin/laporan') ? 'active' : ''}`}>
            <FileSpreadsheet size={18} />
            {!sidebarCollapsed && <span>Laporan & Analitik</span>}
          </Link>

          <Link href="/admin/log" className={`menu-item ${isActive('/admin/log') ? 'active' : ''}`}>
            <Activity size={18} />
            {!sidebarCollapsed && <span>Log Aktivitas</span>}
          </Link>

          <Link href="/admin/pengaturan" className={`menu-item ${isActive('/admin/pengaturan') ? 'active' : ''}`}>
            <Settings size={18} />
        ...
        <div className="sidebar-footer">
          <Link href="/" className="btn-user-site">
